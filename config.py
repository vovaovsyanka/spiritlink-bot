...
class Config:
    ...
    # Настройки постгреса
    DB_USER = "postgres"            # или имя пользователя, которое ты создал
    DB_PASS = quote_plus("eve@123")  # если в пароле спецсимволы — quote_plus их закодирует
    DB_HOST = "127.0.0.1"
    DB_PORT = 5432
    DB_NAME = "spiritlink"          # или ghostbot — имя существующей БД
    ...
    HINTS_BY_MODEL = {
    "Dictionary": [
        "Попробуй использовать больше разных слов, помимо пароля, шифра и тд. (пока я могу дать только такую подсказку)",
    ],
    ...
    "ruBert": [
        "Попробуй добавить опечатки в свою просьбу",
        "Попробуй использовать абстрактные запросы и синонимы подозрительным словам",
    ]
